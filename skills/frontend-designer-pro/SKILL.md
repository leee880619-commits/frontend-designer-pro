...
> 이 §5가 **오케스트레이션 절차의 규범 SSOT**다. `agents/design-lead-pro.md §5`·`agents/design-redteam-pro.md §3·§5`는 각자 역할 관점의 차이만 기술하며, 본 §5와 충돌하면 **본 §5가 우선**한다. 종료 임계는 **3회로 통일**(자동보정 max_retries 3과 일치).
...
- **mandatory_review(영구 산출)**: `TeamCreate`로 **`design-lead-pro` ↔ `design-redteam-pro` 감사 루프**를 강제한다(BLOCK 왕복 ≥2턴). lead가 산출 → redteam이 §7 무결성 게이트로 감사(BLOCK/ASK/NIT) → lead가 BLOCK 수정·재제출 → redteam 재감사. 통과 후 `TeamDelete`로 정리(산출물은 이미 파일로 고정).
  - **산출 규모별 차등(L0~L3)**: redteam이 객관조건(상태 수·실데이터·동적DOM·데이터정직성)으로 등급을 매겨 2차 패스의 **턴 수만** 비례시킨다(L1 경량정적=1턴, BLOCK 발견 시 자동 ≥2턴 승급 / L2·L3=≥2턴). **감사 범위는 어떤 등급도 7영역 전수**(범위 축소 금지), **lead 자가 게이트(§4·§7)는 어떤 출고 등급에서도 면제 불가**. 상세 표·안전조항은 `agents/design-redteam-pro.md §3`(절차 2.5단계).
- **샘플 갤러리(단계 4)**: 경량이라 메인/lead 단독 가능(L0 — redteam 면제). **단 출고물이 아니므로 ①배지(잘 읽힘 등)는 `_audit.html` 실측값만 표기(자기보고 ✓ 금지) ②시안 확정(단계 5) 시 anti-slop P0 + 대비·토큰·CVD(B·C·D) 측정 재진입을 강제**해, 미감사 시안의 슬롭·미달이 본 산출에 기본값으로 상속되는 상류오염을 막는다.
...
- 어느 프리미티브도 없고 `Task`마저 불가한 극단 환경: 메인 세션이 redteam 페르소나로 **역할 전환해 자가 2차 감사**(같은 7영역 체크리스트)를 수행하되, 단일 에이전트 자기검증 한계(spec §9-5)를 무결성 리포트에 명시한다. **이 자가감사 경로는 사용자 신호등을 최대 🟡로 cap(🟢 금지)** — "독립 검토자를 못 띄워 직접 두 번 점검함"을 정직하게 알린다. 극단 환경이라도 "사용자 동의 하 BLOCK 무시 출고"는 불가(불변식 위반) — 🟡 보고 + 보류가 유일 경로다.
...
> - **정리 가드(비정상 종료)**: max_retries 소진·`[BLOCKER]` 정지·교착·세션 종료 — 어떤 사유로 루프를 빠져나가도 `TeamDelete`를 먼저 호출해 `fdp-<요청명>` 팀을 정리한다(산출물은 이미 파일로 고정되어 안전). 재시도 시 동일 이름 팀이 살아있으면 정리 후 재생성한다(리소스 누수·이름 충돌 방지).
...
- **감사 경로 표식**: 보고에 어떤 검토를 거쳤는지 비개발자 카피로 1줄 덧붙인다 — 표준="검토자를 따로 띄워 두 번 점검했어요"(독립 감사) / 강등·자가="이번엔 검토자를 못 띄워 제가 직접 두 번 점검했어요". **자가감사 경로는 신호등 최대 🟡(🟢 금지)** — 독립 검토를 못 받았음을 숨기지 않는다.
