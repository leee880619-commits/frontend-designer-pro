...
> **정본 선언(개정 B-1)**: 이 §5가 **오케스트레이션 절차의 규범 SSOT(정본)**다. `agents/design-lead-pro.md §5`·`agents/design-redteam-pro.md §3·§5`는 각자 역할 관점의 차이만 기술하며, 본 §5와 충돌하면 **본 §5가 우선**한다(절차 3중 기술 드리프트 해소). 종료 임계는 **3회로 통일**(자동보정 max_retries 3과 일치 — 2/3 혼용 금지).

### 5.1 역할 분담
- **메인 세션**: 의도 해석·라우팅·질문·검수·보고만. 직접 `Write`/`Edit`는 **3파일·각 500줄 이하**까지. **단 이 ≤3파일 한도는 출고량 제한일 뿐 감사 면제가 아니다(개정 B-3)** — 직접 Write가 **영구 산출**(=사용자에게 인계되는 모든 산출물, 파일 수 무관)이면 메인은 §5.5 자가 2차 감사(7영역 전수·BLOCK 0건)를 거친 뒤 출고한다. 메모·art brief 등 *비산출 보조파일*만 감사 없는 직접 Write 예외다.
...
- **mandatory_review(영구 산출)**: `TeamCreate`로 **`design-lead-pro` ↔ `design-redteam-pro` 감사 루프**를 강제한다(BLOCK 왕복 ≥2턴). lead가 산출 → redteam이 §7 무결성 게이트로 감사(BLOCK/ASK/NIT) → lead가 BLOCK 수정·재제출 → redteam 재감사. 통과 후 `TeamDelete`로 정리(산출물은 이미 파일로 고정).
  - **산출 규모별 차등(개정 A-3 — L0~L3)**: redteam이 객관조건(상태 수·실데이터·동적DOM·데이터정직성)으로 등급을 매겨 2차 패스의 **턴 수만** 비례시킨다(L1 경량정적=1턴, BLOCK 발견 시 자동 ≥2턴 승급 / L2·L3=≥2턴). **감사 범위는 어떤 등급도 7영역 전수**(범위 축소 금지), **lead 자가 게이트(§4·§7)는 어떤 출고 등급에서도 면제 불가**. 상세 표·안전조항은 `agents/design-redteam-pro.md §3`(절차 2.5단계).
- **샘플 갤러리(단계 4)**: 경량이라 메인/lead 단독 가능(L0 — redteam 면제). **단 출고물이 아니므로(개정 B-7) ①배지(잘 읽힘 등)는 `_audit.html` 실측값만 표기(자기보고 ✓ 금지) ②시안 확정(단계 5) 시 anti-slop P0 + 대비·토큰·CVD(B·C·D) 측정 재진입을 강제**해, 미감사 시안의 슬롭·미달이 본 산출에 기본값으로 상속되는 상류오염을 막는다.

### 5.2 Team 편성 절차 (2개 이상 에이전트 소환 시 필수)
1. `TeamCreate`로 팀 생성(이름은 `fdp-<요청명>` 등 스코프 반영).
2. lead·redteam을 **동일 `team_name`** 으로 **동시 소환**(개정 B-6). redteam은 idle 대기하고, **lead가 산출+무결성 리포트 요약을 보내는 첫 `SendMessage` 수신이 감사 시작 트리거**다("산출 완료 시점에 소환"의 트리거 주체 공백 해소). lead는 background 기본. **lead 산출 후 정해진 대기 내 redteam 기상 ACK가 없으면**(소환 실패·메시지 유실) 부분 장애로 간주 → §5.5 강등 경로로 하강(감사 0턴 좀비 대기 금지).
...
- 어느 프리미티브도 없고 `Task`마저 불가한 극단 환경: 메인 세션이 redteam 페르소나로 **역할 전환해 자가 2차 감사**(같은 7영역 체크리스트)를 수행하되, 단일 에이전트 자기검증 한계(spec §9-5)를 무결성 리포트에 명시한다. **이 자가감사 경로는 사용자 신호등을 최대 🟡로 cap(🟢 금지, 개정 B-8)** — "독립 검토자를 못 띄워 직접 두 번 점검함"을 정직하게 알린다. 극단 환경이라도 "사용자 동의 하 BLOCK 무시 출고"는 불가(불변식 위반) — 🟡 보고 + 보류가 유일 경로다.

> **5.5.1 가용성 런타임 판정 + fail-safe (개정 A-5)** — 환경은 *선언*이 아니라 *런타임 결정*이다.
...
> - **B-5 정리 가드(비정상 종료)**: max_retries 소진·`[BLOCKER]` 정지·교착·세션 종료 — 어떤 사유로 루프를 빠져나가도 `TeamDelete`를 먼저 호출해 `fdp-<요청명>` 팀을 정리한다(산출물은 이미 파일로 고정되어 안전). 재시도 시 동일 이름 팀이 살아있으면 정리 후 재생성한다(리소스 누수·이름 충돌 방지).
...
- **감사 경로 표식(개정 B-8 — 보증을 환경 뒤에 숨기지 않음)**: 보고에 어떤 검토를 거쳤는지 비개발자 카피로 1줄 덧붙인다 — 표준="검토자를 따로 띄워 두 번 점검했어요"(독립 감사) / 강등·자가="이번엔 검토자를 못 띄워 제가 직접 두 번 점검했어요". **자가감사 경로는 신호등 최대 🟡(🟢 금지)** — 독립 검토를 못 받았음을 숨기지 않는다.
...
- **`.fdp/audit-state.json`(개정 B-4·A-4 — 감사 상태 영속)**: mandatory_review 왕복 상태(`gate_status`·`open_blocks`·`resolved_blocks`·`roundtrip_turn`·`tier`·`audit_evidence`·`last_updated`)를 머신 판독용으로 결과 폴더의 **숨김 `.fdp/`** 에 영속한다. **인계 트리(위)에는 노출하지 않지만**(비개발자 혼란 방지·무빌드 약속 유지) 세션 중단 후 재개(§10)를 위해 남긴다. 상태 파일·숨김·인계 제외이므로 §7 무빌드(F) "빌드 잔재" BLOCK 대상이 아니다. 사람용 BLOCK 목록·턴수는 `검증-요약.md`에 임베드하며, 게이트 판정 SSOT는 `audit-state.json`(둘이 충돌하면 json 우선), 사람 설명은 `검증-요약.md`.
...
> 워크플로우 진행 중 세션이 중단되면, 다음 세션 시작 시 메인 세션은 **먼저 `{결과폴더}/.fdp/audit-state.json`의 `gate_status`를 1순위로 확인**하고(개정 B-4·A-4), 없으면 **결과 폴더의 마지막 산출물 파일**로 단계를 추정한다. 사용자에게 "계속/새로 시작"을 확인한 뒤 해당 단계부터 재개한다. **파일이 존재해도 `gate_status`가 `PASS`가 아니면 그 산출물은 미완(감사 미통과)으로 간주**한다 — 파일 존재 ≠ 검증 통과(감사 안 된 산출물을 완성으로 오판하지 않는다).
...
| **`.fdp/audit-state.json`의 `gate_status`≠`PASS`**(미해소 BLOCK 존재) | 단계 7 미통과 | 무결성 게이트(단계 7) 재진입 — `open_blocks` 최우선 해소, **단계 8 진행 금지**. Team은 휘발 가정하고 새로 `TeamCreate` |
