...
10. **경량/본격 모드 자동 분류** — 작업은 객관 신호로 **경량(빠른 모드) / 본격(꼼꼼 모드)을 자동 분류**한다. 분류 규칙: **본격 ⇐ 실데이터 OR 동적DOM OR 웹앱 8상태 OR 데이터정직성(축 잘림·문화적 색 의미) OR C레벨 도메인 OR 큰 표면(다중 섹션·다중 차트·발표 덱); 그 외(단일~소수 화면·정적·예시 데이터)는 경량**. **분류가 애매하면 본격으로 상향**한다(특히 차트가 있으면 '축 0 시작 AND 문화색 중립'을 적극 확신 못 하면 데이터정직성 신호로 보고 본격 — 판단형 신호의 자가 인지 실패에 따른 미탐 방지). 경량=의례(다회 질문·샘플 갤러리·art brief 별도 단계·멀티에이전트 왕복)를 **생략**하고 §7 게이트를 **1회 자가 측정으로 통과**(신호등 최대 🟡). 본격=`design-lead-pro`↔`design-redteam-pro` **독립 왕복**(🟢 가능). 모드는 자동 선택하되 **하드한 첫 질문이 아니라** §2 도메인 확인 질문 안에 ★추천 디폴트로 표면화해, 사용자가 본격으로 올려잡을 수 있게 둔다. **품질 게이트·secure-by-default·데이터정직성 동의(§0-4)는 두 모드 공통이며 생략 불가**(경량은 게이트를 자가 1패스로 *실행*하는 것이지 *skip*이 아니다).
...
### 단계 0 — 진입 & 첨부물 심층 검토
- 메인 세션 진입. 첨부 `.md`/`.html`이 있으면 **Read로 깊게 검토**해 무드·업종·청중·격식·데이터 형태·실행 환경을 선추출하고 **도메인을 자동 추정**한다. **이때 모드(경량/본격)도 객관 신호로 자동 분류**한다(§0-10 규칙: 실데이터·동적DOM·웹앱 8상태·데이터정직성·C레벨·큰 표면 중 하나라도면 본격, 애매하면 본격, 아니면 경량). 분류 결과는 단계 1 도메인 확인에 ★추천 디폴트로 얹는다.
...
- **(경량 모드)** 7문항 묶음이 **도메인 1줄 확인 + 저장위치**로 collapse된다(모드 ★디폴트=빠른 모드는 ①도메인 확인에 얹어 함께 표면화). 나머지 ②~⑦은 ★추천 디폴트를 **조용히 채택**한다. **단 차트+데이터가 함께면 데이터 정직성(축 잘림·문화적 색 의미) 질문은 경량에서도 명시·생략 불가**(§0-4).
...
- **(경량 모드)** art brief를 **별도 단계·별도 산출물로 만들지 않고 산출 안에 인라인으로 접는다**(방향 합의는 단계 1 ★디폴트 수용으로 이미 끝남). 무드·하지 말 것·zone 경계 판단은 그대로 적용하되 1페이지 문서화는 생략.
...
- **(경량 모드)** 샘플은 이미 opt-in이므로 경량에선 **기본 생략**(단계 4 건너뜀). 추천안으로 바로 단계 6 산출.
...
- **(경량 모드)** 토큰 정규화를 **별도 단계로 분리하지 않고 단계 6 산출 안에서 인라인으로 처리**한다(2계층 SSOT·a11y 1급 토큰은 그대로 둠, 정규화를 *생략*하지 않고 *합쳐서* 수행).
...
- **모드별 게이트 실행**: **경량 = `_audit.html` 1회 자가 측정**(Team 소환 없음, 독립 검토자 없음 → §7.3 신호등 최대 🟡 cap + "직접 점검" 라벨). **본격 = `design-lead-pro`↔`design-redteam-pro` 독립 왕복**(BLOCK 왕복 ≥2턴, 🟢 가능, §5.1~5.2). 어느 모드든 **게이트 자체는 BLOCK 0건까지 실행하며 skip 불가**(경량은 자가 1패스로 실행하는 것이지 건너뛰는 게 아니다).
...
- **(경량 모드)** 보고에 **"빠른 모드 — 검토자를 따로 두지 않고 직접 점검했어요"** 라벨을 붙이고 **신호등은 최대 🟡**(독립 검토를 못 받았음을 정직하게 표시, §7.3). 본격 모드는 "검토자를 따로 띄워 두 번 점검했어요" 라벨 + 🟢 가능.
...
- **맨 처음 도메인 확인 질문에 모드도 ★추천 디폴트로 함께 표면화**한다 — 한 화면짜리 간단한 작업이면 "★빠른 모드", 보고서·발표 자료·임원용·실제 데이터가 들어가는 작업이면 "★꼼꼼 모드"(독립 검토). 별도의 하드한 첫 질문으로 분리하지 않고, 사용자는 언제든 더 꼼꼼한 모드로 올릴 수 있다.
- **(경량 모드 묶음 축소)** 경량으로 분류되면 7문항 묶음이 **도메인 확인 + 저장위치**(+차트+데이터면 데이터 정직성)로 collapse되고, 나머지는 ★추천 디폴트를 조용히 채택한다(§1 단계 1과 일치).
...
1. **[도메인 확인 + 모드]** "첨부 내용을 보니 임원 보고용 자료 같습니다. 맞을까요? (예 ★ / 아니요-웹앱 / 아니요-발표 슬라이드) — 임원 보고서는 화려함보다 신뢰감·가독·절제를 우선해 만듭니다." 같은 칸에 모드도 ★디폴트로 곁들인다: "만드는 방식은 (빠른 모드=바로 만들고 제가 직접 두 번 점검 / ★꼼꼼 모드=검토자를 따로 띄워 독립으로 두 번 점검) — 임원 보고서는 중요한 자료라 ★꼼꼼 모드를 추천합니다(한 화면짜리 간단한 작업이면 빠른 모드로 충분). 나중에 바꿀 수 있어요."
...
> **§4.6 주 — Python 웹앱 운영 경로 (신규/개선 양분)**: ⓐ **신규 제작**이면 단계 2~5는 동일(art brief·샘플·토큰 정규화)하되, 단계 6에서 `tokens.css` 대신 **프레임워크 테마 산출물**(`​.streamlit/config.toml`·`theme.py`·`assets/custom.css`·`templates/*.html`)로 토큰을 매핑한다. ⓑ **기존 앱 개선**이면 → ①대상 앱을 Read로 읽어 프레임워크·현재 테마·위젯 사용을 파악 → ②art brief로 목표 룩 합의(단계 1~2) → ③**단일 `styles.css`/테마 객체 1회 주입**으로 룩만 바꾸고 **파이썬 로직·데이터 흐름은 불변** → ④§9-10 원본 보존(`_legacy/`) → ⑤§7 무결성 게이트는 *렌더된 화면 기준*으로 적용(위젯 기본 대비·키보드·CVD 점검; 무빌드 더블클릭 항목만 면제). 산출물·검증요약은 "테마 파일 + 적용법 + 신호등"으로 보고한다(index.html 핸드오프 아님).

---

## 5. 런타임 오케스트레이션

> 이 §5가 **오케스트레이션 절차의 유일 정본(SSOT)**이다. `agents/design-lead-pro.md §5`·`agents/design-redteam-pro.md §3·§5`는 본 §5를 **참조만 하고 절차를 재기술하지 않으며**, 충돌하면 **본 §5가 우선**한다. 종료 임계는 **3회로 통일**(자동보정 max_retries 3과 일치).
>
> **모드는 둘뿐 — {경량(light) / 본격(full)}.** 산출 규모가 아니라 **객관 조건**으로 가른다. **full** ⇐ 다음 중 하나라도면: 실데이터 투입·동적 DOM·웹앱 인터랙티브 8상태·데이터정직성(축잘림·문화색)·C레벨 도메인·큰 표면(다중 섹션·다중 차트·발표 덱). **light** ⇐ 위 어느 것도 아니고 **단일~소수 화면·정적·예시데이터**(랜딩 한 장·카드·단순 안내처럼 표면이 작은 일). **분류가 애매하면 full로 상향**한다(차트가 있는데 '축 0 시작 AND 문화색 중립'을 적극 확신 못 하면 데이터정직성 신호로 보고 full — 판단형 신호 미탐 방지). 작고 안전한 일만 **light가 자동 디폴트**다. 분류는 산출자(메인 세션/lead)가 하고, **full이면 redteam이 재검증**, **light면 산출자가 자가 1패스**로 점검한다(아래 §5.1).
>
> - **light 파이프라인(최단·약속 유지)**: 1회 질문게이트(★디폴트 일괄)+도메인 1줄 확인+저장위치 확인(차트+데이터면 **데이터정직성 1문항 명시·생략불가**) → secure-by-default 자동 ON·art brief 암묵 → 샘플 생략 → 6 렌즈 인라인 산출 → **§7 무결성 게이트 1회 자가 측정(`${CLAUDE_PLUGIN_ROOT}/tools/_audit.html`): anti-slop·토큰lint·APCA AND WCAG·axe/CVD, 자동보정 ≤3회, Team/redteam 왕복 없음, 범위는 7영역 전수(축소 없음)** → 보고 신호등 **최대 🟡(🟢 금지)** + 정직 라벨("빠른 모드 — 검토자를 따로 두지 않고 직접 점검했어요"). `.fdp/audit-state.json`에 `mode:"light"` 기록.
> - **full 파이프라인(현행 유지·이름만 'full')**: `TeamCreate` → lead↔redteam ≥2턴 왕복(`SendMessage`) → BLOCK 0 → `TeamDelete` → 🟢 가능. §5.5 폴백(Task 강등·극단환경 자가감사)은 full 경로의 가용성 폴백으로 유지. `.fdp/audit-state.json`에 `mode:"full"` 기록.

### 5.1 역할 분담
- **메인 세션**: 의도 해석·라우팅·질문·검수·보고만. 직접 `Write`/`Edit`는 **3파일·각 500줄 이하**까지. **단 이 ≤3파일 한도는 출고량 제한일 뿐 감사 면제가 아니다** — 직접 Write가 **영구 산출**(=사용자에게 인계되는 모든 산출물, 파일 수 무관)이면 메인은 §5.5 자가 2차 감사(7영역 전수·BLOCK 0건)를 거친 뒤 출고한다. 메모·art brief 등 *비산출 보조파일*만 감사 없는 직접 Write 예외다.
...
- **mandatory_review(영구 산출)**: `TeamCreate`로 **`design-lead-pro` ↔ `design-redteam-pro` 감사 루프**를 강제한다(BLOCK 왕복 ≥2턴). lead가 산출 → redteam이 §7 무결성 게이트로 감사(BLOCK/ASK/NIT) → lead가 BLOCK 수정·재제출 → redteam 재감사. 통과 후 `TeamDelete`로 정리(산출물은 이미 파일로 고정).
  - **모드 분기(binary {light/full})**: 산출자(메인 세션/lead)가 객관 조건(실데이터·동적DOM·웹앱 8상태·데이터정직성·C레벨·큰 표면[다중 섹션·다중 차트·덱])으로 모드를 분류한다 — 하나라도면 **full**, 아니면 **light**(단일~소수 화면·정적·예시데이터). **분류가 애매하면 full로 상향**(uncertainty→full; 차트가 있는데 정직성 적극 확신 불가면 full). **full**이면 redteam이 §7 무결성 게이트로 **재검증**(이 mandatory_review 루프), **light**면 산출자가 **단일 자가 패스**로 자가 점검(Team/redteam 왕복 없음, 신호등 🟡 cap). **감사 범위는 어떤 모드도 7영역 전수**(범위 축소 금지), **자가 게이트(§4·§7)는 어떤 모드에서도 면제 불가**. 상세 표·안전조항은 `agents/design-redteam-pro.md §3`(절차 2.5단계).
- **샘플 갤러리(단계 4)**: 탐색 샘플 = **출고물 아님** → 메인/lead 단독 가능(**redteam 면제**). **단 ①배지(잘 읽힘 등)는 `_audit.html` 실측값만 표기(자기보고 ✓ 금지) ②시안 확정(단계 5) 시 light/full로 재진입하며 anti-slop P0 + 대비·토큰·CVD(B·C·D) 측정을 강제**해, 미감사 시안의 슬롭·미달이 본 산출에 기본값으로 상속되는 상류오염을 막는다.

### 5.2 Team 편성 절차 (2개 이상 에이전트 소환 시 필수)
1. `TeamCreate`로 팀 생성(이름은 `fdp-<요청명>` 등 스코프 반영).
2. lead·redteam을 **동일 `team_name`** 으로 **동시 소환**. redteam은 idle 대기하고, **lead가 산출+무결성 리포트 요약을 보내는 첫 `SendMessage` 수신이 감사 시작 트리거**다. lead는 background 기본. **lead 산출 후 정해진 대기 내 redteam 기상 ACK가 없으면**(소환 실패·메시지 유실) 부분 장애로 간주 → §5.5 강등 경로로 하강(감사 0턴 좀비 대기 금지).
...
> **이 §5.5는 full 모드(Team을 띄우는 경로)의 가용성 폴백 사다리다.** light 모드는 Team을 띄우지 않으므로(산출자 자가 1패스) 정리할 Team이 없고, 아래 강등 사다리의 최종단(자가 2차 감사)이 곧 **light가 쓰는 것과 동일한 단일 자가 패스 메커니즘**이다(🟡 cap 동일).

`TeamCreate`/`TeamDelete`/`SendMessage`/`TaskCreate`/`TaskUpdate`/`run_in_background`는 **full 모드 mandatory_review 감사 루프의 안전망**이다. 이 프리미티브가 환경마다 보장되지 않으므로 **두 경로로 양분**한다.
...
**강등 시 동작·한계(반드시 인지)**:
- 강등 경로의 redteam은 **별도 컨텍스트의 1회성 서브에이전트**라, Team처럼 같은 세션 내 ≥2턴 상태를 자연히 잇지 못한다. → 매 패스마다 redteam에 **고정 5필드 핸드오프**를 명시적으로 다시 넣어 상태를 수동 전달한다: ①산출물 경로 ②`open_blocks`(직전 미해소 BLOCK 전체 — `.fdp/audit-state.json`에서 인용) ③`resolved_blocks`(이번 패스 수정 주장) ④`_audit.html` 측정 표(지문·raw 색쌍 포함) ⑤"7영역 전수 재점검 의무(회귀 포함)". 한 필드라도 누락하면 회귀가 무검출 통과하는 fail-open이 생긴다.
- 강등 경로에서도 **`${CLAUDE_PLUGIN_ROOT}/tools/_audit.html` 측정 표를 redteam 입력 증거로 첨부**하는 원칙은 동일(redteam은 코드 미실행이며, §4 독립 손계산 검산도 동일 적용).
- **공통 불변식**: 경로가 무엇이든 BLOCK 0건 전에는 출고 금지(mandatory_review, skip 불가). 폴백이라고 감사를 생략하지 않는다.
- 어느 프리미티브도 없고 `Task`마저 불가한 극단 환경: 메인 세션이 **single 자가 패스**(light 모드와 동일한 단일 자가 감사 메커니즘)로 같은 7영역 체크리스트를 전수 점검하되, 단일 에이전트 자기검증 한계(spec §9-5)를 무결성 리포트에 명시한다. **이 자가감사 경로는 light 모드와 마찬가지로 사용자 신호등을 최대 🟡로 cap(🟢 금지)** — "독립 검토자를 못 띄워 직접 두 번 점검함"을 정직하게 알린다. 극단 환경이라도 "사용자 동의 하 BLOCK 무시 출고"는 불가(불변식 위반) — 🟡 보고 + 보류가 유일 경로다.

> **5.5.1 가용성 런타임 판정 + fail-safe** — 환경은 *선언*이 아니라 *런타임 결정*이다.
> - **판정**: `TeamCreate` 시도 → 실패(도구 부재·에러)면 강등. 성공했으나 redteam 첫 회신이 **상태 기준**(절대 시간 아님 — "redteam의 최초 회신 메시지 수신 전까지는 게이트 미통과로 유지")으로 안 오면 부분 장애 → `TeamDelete` 후 강등.
> - **fail-safe 불변식**: 가용성이 불확실하면 **항상 '감사 수행' 쪽으로 결정**한다. **회신 없음을 PASS로 해석하지 않는다**(fail-open 금지). 강등으로 끝까지 하강해도 감사는 반드시 1회 이상 수행한다.
> - **정리 가드(비정상 종료) — full 모드에만 적용**: full 모드는 Team을 띄우므로, max_retries 소진·`[BLOCKER]` 정지·교착·세션 종료 — 어떤 사유로 루프를 빠져나가도 `TeamDelete`를 먼저 호출해 `fdp-<요청명>` 팀을 정리한다(산출물은 이미 파일로 고정되어 안전). 재시도 시 동일 이름 팀이 살아있으면 정리 후 재생성한다(리소스 누수·이름 충돌 방지). **light 모드는 Team을 띄우지 않으므로 정리할 대상이 없다.**
...
- **감사 경로 표식 + 모드 cap**: 보고에 어떤 검토를 거쳤는지 비개발자 카피로 1줄 덧붙인다 — **본격(꼼꼼 모드)·표준 독립 감사**="검토자를 따로 띄워 두 번 점검했어요" / **경량(빠른 모드)·강등·자가 감사**="이번엔 검토자를 따로 두지 않고 제가 직접 두 번 점검했어요". **자가감사(=경량 1회 자가 측정, 또는 환경 강등) 경로는 전체 신호등을 최대 🟡로 cap(🟢 금지)** — 독립 검토를 못 받았음을 숨기지 않는다. **🟢은 본격(독립 검토) 경로에서만 가능**하고, 빠른 모드는 게이트를 통과해도 🟡로 정직하게 보고한다. 개별 축의 🟢/🟡/🔴 대비 raw 결과는 `검증-요약.md` 백스테이지에 두고, **경량 모드의 사용자 표면 신호등은 전체를 🟡로 표기**해 비개발자가 축별 🟢을 전체 통과로 오인하지 않게 한다.
...
- **`.fdp/audit-state.json`(감사 상태 영속)**: mandatory_review 왕복 상태(`gate_status`·`open_blocks`·`resolved_blocks`·`roundtrip_turn`·`mode`("light"|"full")·`audit_evidence`·`last_updated`)를 머신 판독용으로 결과 폴더의 **숨김 `.fdp/`** 에 영속한다. **인계 트리(위)에는 노출하지 않지만**(비개발자 혼란 방지·무빌드 약속 유지) 세션 중단 후 재개(§10)를 위해 남긴다. 상태 파일·숨김·인계 제외이므로 §7 무빌드(F) "빌드 잔재" BLOCK 대상이 아니다. 사람용 BLOCK 목록·턴수는 `검증-요약.md`에 임베드하며, 게이트 판정 SSOT는 `audit-state.json`(둘이 충돌하면 json 우선), 사람 설명은 `검증-요약.md`.
...
> 워크플로우 진행 중 세션이 중단되면, 다음 세션 시작 시 메인 세션은 **먼저 `{결과폴더}/.fdp/audit-state.json`의 `gate_status`를 1순위로 확인**하고, 없으면 **결과 폴더의 마지막 산출물 파일**로 단계를 추정한다. 사용자에게 "계속/새로 시작"을 확인한 뒤 해당 단계부터 재개한다. **파일이 존재해도 `gate_status`가 `PASS`가 아니면 그 산출물은 미완(감사 미통과)으로 간주**한다 — 파일 존재 ≠ 검증 통과(감사 안 된 산출물을 완성으로 오판하지 않는다).
...
| **`.fdp/audit-state.json`의 `gate_status`≠`PASS`**(미해소 BLOCK 존재) | 단계 7 미통과 | 무결성 게이트(단계 7) 재진입 — `open_blocks` 최우선 해소, **단계 8 진행 금지**. `mode`가 `full`이면 Team은 휘발 가정하고 새로 `TeamCreate`; `light`면 자가 1패스로 재점검 |
| 검증 리포트 있음, 인계 보고 미완 | 단계 7 완료 | 저장·인계 보고(단계 8) |

- 재개 시 **이미 확정된 결정(도메인·미적 방향·실행환경)은 다시 묻지 않는다**(art brief·메모에서 복원). 미해결 항목만 이어서 처리한다.
- 중단 직전 BLOCK 상태였다면(무결성 게이트 미통과) — `.fdp/audit-state.json`의 `open_blocks`(또는 `검증-요약.md`에 임베드된 BLOCK 목록)로 복원해 그 항목을 최우선 해소한 뒤 재산출한다 — skip 불가. 목록이 파일에 없으면 redteam 7영역 전수 재감사로 안전하게 재구성한다(fail-safe — 진행 중 BLOCK 유실 시 처음부터 재감사).
