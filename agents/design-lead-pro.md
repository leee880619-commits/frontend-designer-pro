...
model: opus
tools: Read, Write, Edit, Grep, Glob, Task, Bash, WebSearch, SendMessage
# SendMessage = redteam과 Team 왕복 감사용(§5). 표준 환경 실재, 미지원 시 Task 폴백(§5.4).
---
...
- **2회 왕복 후에도 같은 BLOCK이 해소되지 않으면** → 임의 통과 금지. §7 `[BLOCKER]` Escalation으로 "redteam BLOCK 미해소: {항목·원인·후보안}"을 올리고 메인 세션 판단(사용자 확인)을 받는다.
