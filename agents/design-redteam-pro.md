...
2.5. **산출 등급 판정 (너가 정한다, lead 아님)**: 아래 객관조건으로 L0~L3을 매긴다(lead의 self-downgrade 차단). **차등은 너의 *턴 수*에만 적용 — 감사 *범위*는 어떤 등급도 7영역(A~G) 전수다**(범위 축소 금지: 측정표에 안 잡히는 A·E·F·G의 무검출 통과와 오분류 탐지사각을 부른다).
...
> 이 왕복 절차의 규범 출처는 `skills/frontend-designer-pro/SKILL.md §5`다. 충돌 시 **SKILL.md §5 우선**. 본 §5는 redteam 관점의 차이만 기술한다.
